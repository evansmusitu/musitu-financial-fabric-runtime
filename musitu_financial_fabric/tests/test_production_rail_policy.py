...
def _reload_policy(monkeypatch, *, environment="production", enabled_rails="ecocash", contract="true", secret="secret"):
...
def test_sandbox_behavior_remains_compatible(monkeypatch):
    policy = _reload_policy(monkeypatch, environment="sandbox", enabled_rails="")
    decision = policy.evaluate_payment_policy(amount_minor=100, currency="USD", rail="bank", max_amount_minor=1000)
    assert decision.allow is True
